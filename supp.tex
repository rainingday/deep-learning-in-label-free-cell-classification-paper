...
% (a) Principal component analysis (PCA). The percentage of the total variance explained by each principal component.
% (b) Accuracy as increasig PCA components.
% (c) Running time as decreasing dimensions.
\end{figure*}

\subsection{Cross validation}

The k-fold cross-validation implemented here splits data points into training, validation, and test subsets (Fig.~\ref{CrossVaidationRegularization}a). For each interation, one fold is used as test data, one for validation while the other folds are used during training process. After innitially trained, the performance of the network is analyzed by the validation data to fine tune the neural network architecture and regularization parameter. Fig.~\ref{CrossVaidationRegularization}b shows that either a too small or a too large regularization parameter, $\lambda$, increases network error due to overfitting or underfitting, respectively. Therefore, there is a suitable range of regularization parameter for each learning model.
...
% \begin{figure*}
% % \includegraphics[scale=0.18]{FigureCrossVaidationRegularization.jpg}
% \caption{\label{fig:CrossVaidationRegularization} 
% (a) The k-fold cross-validation implemented here splits data points into training, validation, and test subsets. The final reported results are aggregate of the performance for different test datasets. (b) Either a too small or a too large regularization parameter, $\lambda$, increases network error due to overfitting or underfitting, respectively. There is a suitable range of regularization parameter for each learning model.
% \end{figure*}
