...
&\quad\quad\quad \cdot \exp \Big\lbrace i\Big[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t_p)\Big] \Big\rbrace\\
...
&\quad \cdot \exp \Big\lbrace i[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t_p)] \Big\rbrace \\
...
&\quad - \Delta\varphi(x,t_p)] \rbrace - [1-T_b(\omega)] \Big\rbrace.
\end{split}
\end{equation}
...
\begin{split}
\tilde{I}_o(\omega, t_p) &\propto \Big\lbrace [1-T_b(\omega)]^2 + T_b(\omega)^2 T_c(\omega, t_p)^2 T_s(\omega)^2\Big\rbrace R_m(\omega) \\
...
\end{equation}
where $\lambda_c$ is the central wavelength and $L_f$ the length of dispersive fiber. $D$ is the group velocity dispersion, that is, the temporal pulse spreading $\Delta t_i$ per unit bandwidth $\Delta\lambda$ per unit distance travelled. Thus the energy flux absorbed at the photodetector is the vertical concatenation of inter-pulse waveforms:
\begin{equation}
\begin{split}
\tilde{I}_{PD}(t) &=\\
\text{vetcat} &
