...
\end{split}
\end{equation}
where $T_b$ is the power transmission ratio of the beam-splitter. Optical intensity in the sample arm will be altered by the absorption and scattering of imaged cells, as well as that of the microfluidic channel and buffer solution. Not only the electric field amplitude after passing through semitransparent objects will be modulated by the optical absorption and scattering in the sample arm, but also the optical path length difference will lead to a phase shift, $\Delta\varphi(x,t)$, inducted by refractive index change from the object along the interrogation beam. The complex fields of the light waves coming back to the beam splitter become
\begin{equation}
\begin{split}
&\text{From the sample arm:}\\
&\quad \tilde{E}_s(\omega, t_p) = \sqrt{T_s(\omega) T_c(x,t_p) T_b(\omega)} \sqrt{R_m(\omega)} \tilde{E}_in(\omega, t_p) \exp \lbrace i[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t)] \rbrace\\
&\text{From the reference arm:} \\
&\quad \tilde{E}_r(\omega, t_p) = i \sqrt{1-T_b(\omega)} \sqrt{R_m(\omega)} \tilde{E}_{in}(\omega, t_p) \exp[- i \frac{\omega}{c}\cdot 2L]
\end{split}
\end{equation}
where $T_s$ is transmittance of the surrounding buffer solutions and microfluidic channel, $T_c(x,t)$ is transmittance of cells at location $x$ along the rainbow at time $t$ in the field of view,; $L_0$ is the length of reference arm, and $\Delta L$ is the arm length mismatch between two arms. The total electric field at each wavelength after two arms of the interferometer recombine at the beam splitter becomes
\begin{equation} \label{eq:totalElecField}
\begin{split}
E_o(\omega, t_p) &= \sqrt{T_s(\omega) R_m(\omega)} T_c(x, t_p) T_b(\omega) \tilde{E}_in(\omega, t_p) \exp \lbrace i[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t)] \rbrace + 
- [1-T_b(\omega)] \sqrt{R_m(\omega)} \tilde{E}_{in}(\omega, t_p) \exp[- i \frac{\omega}{c}\cdot 2L] \\
&= \sqrt{R_m(\omega)} \tilde{E}_{in}(\omega, t_p) \exp(- i \frac{2\omega L}{c}) \lbrace \sqrt{T_s(\omega)} T_c(x, t_p) T_b(\omega) exp[i(-2\frac{\omega}{c}\Delta L  \\
&\quad - \Delta\varphi(x,t))] - [1-T_b(\omega)] \rbrace.
\end{split}
\end{equation}
The intensity envelop is
\begin{equation} \label{eq:totalIntensity}
\begin{split}
I_o(\omega, t_p) &\propto \left| \tilde{E}_{in}(\omega, t_p)\right|^2 \lbrace [1-T_b(\omega)]^2 + T_b(\omega)^2 T_c(\omega, t_p)^2 \rbrace ^2 R_m(\omega) \\
& - 2 [1-T_b(\omega)] R_m(\omega) T_c(x, t_p) T_b(\omega) \left| \tilde{E}_{in}(\omega, t_p)\right|^2 cos[ 2\omega \Delta L / c + \Delta \varphi_c(\omega, t_p) ]
\end{split}
\end{equation}
...
After time stretch, the frequency components in $\omega_s$ and $\omega_{LO}$, are mapped into time. That is, electric field detected at time t is one-to-one mapped from frequency domain,
\begin{multline}
