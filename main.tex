...
&\quad\quad\quad\quad + 2k(\lambda)(L_0+\Delta L)] \rbrace\\
...
&\quad+ 2k(\lambda)(L_0+\Delta L)] \rbrace + \sqrt{1-r^2} E_i(t_p,\omega_{LO})\\
...
\multicolumn{1}{ |m{0.15\textwidth}| }{Diffraction gratings} & \begin{equation} N_{grating} = \frac{\Delta\lambda}{\delta\lambda_{grating}} =\Delta\lambda /(\lambda \cdot \frac{d}{m\cdot2w_0})\end{equation} where $\Delta\lambda$ is the optical bandwidth, $\lambda$ as the central wavelength, m is the order of the diffraction, $w_0$ is the beam width, and $d$ is the groove spacing. &$\SI{3.09}{\micro\meter}$ \\ \cline{2-4}
