...
\begin{figure*}
\includegraphics[scale=0.5]{Figure2DImage.pdf}
...
\begin{figure*}
\includegraphics[scale=0.5]{FigureOTSWScatter.pdf}
...
\begin{figure*}
\includegraphics[scale=0.6]{FigureCorrelation.pdf}
...
\begin{figure*}
\includegraphics[scale=0.55]{FigureFeaturesRank.pdf}
...
\begin{figure*}
\includegraphics[scale=0.55]{FigureNeuralNet.pdf}
...
\begin{figure*}
\includegraphics[scale=0.55]{FigureOTSWROC.pdf}
...
\begin{figure*}
\includegraphics[scale=0.6]{FigurePCA.pdf}
...
\begin{figure*}
\includegraphics[scale=0.55]{FigureAlgae.pdf}
...
\subsection{Correction and Calibration for Phase Extraction}

For high-throughput quantitative imaging, we have to do a phase correction when applying Hilbert Transform because the intensity variation induced by the passing cells adds up low frequency components to the spectrum in detection domain. Most cells with clinical or industrial interests have a diameter about 3~40 µm when suspended in fluid. Given the field of view of the interrogation rainbow is 120 µm, cells are coming at a speed of 2 m/s, the instantaneous intensity change induced by single cell in each laser pulse is about 0.5~9.0 ns, which will bring in frequency components below 1.5 GHz. Compared to the higher frequency components at 5 GHz corresponding to the interference fringes, Hilbert transform is robust in this scenario to separate the two frequency terms for optical loss and phase. 

Two other major effects need to be calibrated since they may alter the quantitative phase extraction based on Hilbert Transform: the inbalanced power distribution at the beam splitter in the Michaelson Interferometry, and the attenuation inbalance caused by microfluidic channel. When taking the inbalanced power distribution at the beam splitter into consideration, the light splits into the two arms of the Michaelson Interferometry can be expressed as: 
\begin{equation}
\begin{split}
&\text{Into the sample arm:} \\
&\quad E_s(t) = rE_i(t)\exp(i\omega_{s} t)  \\
&\text{Into the reference arm:} \\
&\quad E_{LO}(t) = \sqrt{1-r^2}E_i(t)\exp(i\omega_{LO} t) 
\end{split}
\end{equation}
where $r$ is the reflectance at the beam-splitter. $E_s(t)$, $E_{LO}(t)$, $\omega_s$ and $\omega_{LO}$ are same as what mentioned above. Since biological cells are semi-transparent, optical intensity in the sample arm will be altered not only by the absorption and scattering caused by imaged objects, but also by that of the microfluidic channel. The complex fields of the light combining at the beam splitter become 
\begin{equation}
\begin{split}
&\text{From the sample arm:}\\
&\quad E_s(t) = \alpha_s \alpha_c(x,t) r E_i(t) \exp \lbrace i[\omega_s t + \Delta\varphi(x,t) \\
&\quad\quad\quad\quad + 2k(\lambda)L_0] \rbrace\\
&\text{From the reference arm:} \\
&\quad E_{LO}(t) = \sqrt{1-r^2} E_i(t)\exp(i\omega_{LO} t + 2k(\lambda)L_0)
\end{split}
\end{equation}
where $\alpha_s$ is 1 - absorption coefficient of the surrounding buffer solutions and microfluidic channel, and $\alpha_c(x,t)$ is 1 - absorption coefficient of cells at location $x$. The total amplitude at each wavelength after recombining becomes
\begin{equation}
\begin{split}
E_{total}(t) &= \alpha_s \alpha_c(x,t) r E_i(t_p,\omega_s) \exp \lbrace i[\omega_s t + \Delta\varphi(x,t) \\
...
&\quad+ \alpha_s \alpha_c(x,t) r \exp [i\Delta\varphi(x,t)] \rbrace
\end{split}
\end{equation}
assuming $E_{LO}(t_p,\omega_{LO}) \approx E_s(t_p,\omega_s)$ according to equation~\ref{eq:sandLO}. The intensity envelop is
\begin{equation}
\begin{split}
I_{total}(t) &= E_i^2(t_p,\omega_{s}) \lbrace (1-r^2) + \alpha_s^2 \alpha_c^2(x,t) r^2 \\
&+ 2r\sqrt{1-r^2} \alpha_s \alpha_c cos[i\Delta\varphi(x,t)] \rbrace
\end{split}
\end{equation}
Thus we can clearly get the intensity envelop under two scenarios would be
\begin{equation}
I_{env}(t) = \begin{cases}
E_i^2(t_p,\omega_{s}) [(1-r^2) + \alpha_s^2 r^2], \\
\quad\quad \text{empty frames}.\\
E_i^2(t_p,\omega_{s}) [(1-r^2) + \alpha_s^2 \alpha_c^2(x,t) r^2], \\
...
\begin{equation}
\Delta I_{env}(t) =
 \frac{\alpha_s^2 r^2 [\alpha_c^2(x,t) - 1]}{1 - r^2 + \alpha_s^2 r^2}
\end{equation}
Define $r^\prime=\frac{r\alpha_s}{\sqrt{1-r^2+r^2\alpha_s^2}}$. Then the ratio of amplitude change caused by interference to that of envelop change caused by cell absorption:
\begin{equation}
\begin{split}
\beta &= \frac{I_{intf}}{I_{env}} \\
&= \frac{2 \alpha_s \alpha_c cos[i\Delta\varphi(x,t)]}{\frac{\sqrt{1-r^2}}{r} + \frac{\alpha_c^2(x,t) \alpha_s^2 r}{\sqrt{1-r^2}}} 
= \frac{2 \alpha_c cos[i\Delta\varphi(x,t)]}{\frac{\sqrt{1-r^{\prime2}}}{r^\prime} + \frac{\alpha_c^2(x,t) r^\prime}{\sqrt{1-r^{\prime2}}}}
\end{split}
\end{equation}
...
\begin{table*}
\caption{\label{tbl:Resolution} Deterministic Factors in TS-QPI}
\begin{tabular}{|m{0.15\textwidth}|m{0.15\textwidth}|m{0.45\textwidth}|m{0.10\textwidth}|}
 \hline
 System catagory   &Component  &Number of resovable points &Lateral resolution    \\  \hline
\multicolumn{1}{ |m{0.15\textwidth}  }{\multirow{2}{*}{Optical system} } &
\multicolumn{1}{ |m{0.15\textwidth}| }{Diffraction gratings} & \begin{equation} N_{grating} = \frac{\Delta\lambda}{\delta\lambda_{grating}} =\Delta\lambda /(\lambda \cdot \frac{d}{m\cdot2w_0})\end{equation} where m is the order of the diffraction, $w_0$ is the beam width, $\lambda$ as the central wavelength, and $d$ is the groove spacing. &$\SI{3.09}{\micro\meter}$ \\ \cline{2-4}
...
\multicolumn{1}{ |m{0.15\textwidth}  }{\multirow{1}{*}{Time Stretch} } &
\multicolumn{1}{ |m{0.15\textwidth}| }{Group delay dispersion} &\begin{equation}N_{DFT}=\frac{\Delta\lambda}{\delta\lambda}=\frac{\Delta\lambda}{\lambda\cdot\sqrt{\frac{2}{DL_f\cdot c}}}\end{equation} where $D$ is the group velocity dispersion, $L_f$ is the dispersion fiber length. &$\SI{0.73}{\micro\meter}$ \\ \cline{1-4}
\multicolumn{1}{ |m{0.15\textwidth}  }{\multirow{2}{*}{Electronic back-end} } &
\multicolumn{1}{ |m{0.15\textwidth}| }{Photodetector bandwidth} &\begin{equation}N_{PD}=\frac{\Delta t}{\delta t}=\frac{DL_f\cdot\Delta\lambda}{0.35/B}\end{equation} where $B$ is the bandwidth of the photodetector. &$\SI{0.28}{\micro\meter}$ \\ \cline{2-4}
\multicolumn{1}{ |m{0.15\textwidth}  }{} &
...
field of view: Optical Bandwidth $\Delta\lambda$
Pulse to pulse intensity and phase fluactuation is small. Therefore it is not limiting factor for resolvable points. Decide frame rate and shutter speed. 

 Limited by the smallest resovable wavelength difference. Define m as the order of the diffraction, $w_0$ as the beam width, $\lambda$ as the central wavelength, $\Delta\lambda$ as the optical bandwidth, $d$ as the groove spacing. 

Field of view is decided by the width of the interrogation rainbow when hitting the target. Lateral resolution is limited by diffraction limit of the objective lens. Define $NA$ as the numerical apeture of the objective lens and FOV as the field of view, 

Lateral resoluiton is limited by spectral resolution in amplified dispersive Fourier transform, photodetector rise-time and analog-to-digital converter bandwidth.

The axial resolution for optical path length measurement is limited by the effective number of bits (ENOB) of the digitizer: 
\begin{equation}
\frac{1}{2}sin(4\pi \Delta L/\lambda)=2^{-N}
\end{equation}
where $N$ is the ENOB of the digitizer, $\lambda$ is the central wavelength of light, and $\Delta L$ is the minimum detectable optical path length. In our system, ENOB of the analog-to-digital converter is 5. Thus OPD resolution is $7.9 nm$, corresponding to refractive index difference about $1^{-3}$ for cellular level measurements.
