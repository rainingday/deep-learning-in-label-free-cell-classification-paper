...
Since the wavelength are mapped into space by diffraction gratings, the complex field at any specific spatial location within the field of view is a narrow-band optical pulse wave. As the envelope of the optical pulse wave varies slowly in time compared to the mismatch period between the reference arm and sample arm, we use slowly varying envelope approximation (SVEA) for the assumption. The complex envelope of the input electric field, $\tilde{E}_{in}(\omega, t_p)$, is split into two arms of the Michaelson interferometer at the beam splitter. Here, $\omega$ is the optical frequency of the input signal, which is corresponding to the spatial location $x$ being interrogated by the optical pulse at this frequency. This unique feature is called spectral encoding. Each $t_p$ specifies the time of each rainbow flash when it reaches the beam splitter, corresponds to the $p$-th incoming pulse. Note that $\tilde{E}_{in}(\omega,t_p)$ can be simplified as $\tilde{E}_{in}(\omega)$ when pulse shape (magnitude of each wavelength) is stable from pulse to pulse. The light split into the two arms of the Michaelson interferometer can be expressed as
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $T_b$ is the power transmission ratio of the beam-splitter. Optical intensity in the sample arm will be altered by the absorption and scattering of imaged cells, as well as that of the microfluidic channel and buffer solution. Not only the electric field amplitude after passing through semitransparent objects will be modulated by the optical absorption and scattering in the sample arm, but also the optical path length difference will lead to a phase shift, $\Delta\varphi(x,t_p)$, inducted by refractive index change from the object along the interrogation beam. The complex fields of the light waves coming back to the beam splitter become
\begin{equation}
\begin{split}
&\text{From the sample arm:}\\
&\quad \tilde{E}_s(\omega, t_p) = T_s(\omega) T_c(x,t_p + t_d) \sqrt{T_b(\omega) R_m(\omega)} \tilde{E}_{in}(\omega, t_p) \\
&\quad\quad\quad \cdot \exp \Big\lbrace i\Big[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t)\Big] \Big\rbrace\\
...
\end{split}
\end{equation}
where $L_0$ is the length of reference arm, and $\Delta L$ is the arm length mismatch between two arms. $t_d$ is the time delay during which rainbow flash travels from the beam splitter to the cell sample, $t_d = \frac{L_0 + \Delta L}{2}$. $T_s(\omega)$ is power transmittance of the surrounding buffer solutions and microfluidic channel, and $T_c(x,t_p + t_d)$ is spatial power transmittance of cells at location $x$ along the rainbow when being illuminated at time $t_p + t_d$. Both $T_s(\omega)$ and $T_c(x,t_p + t_d)$ have been multiplied twice since each rainbow flash pass through objects twice. As the optical wave meets the requirements of slowly varying envelop approximation, we can approximate $T_c(x,t_p + t_d)$ to be $T_c(x,t_p)$ to synchronize with \tilde{E}_{in}(\omega, t_p) without sacrificing accuracy.
The total electric field at each wavelength after two arms of the interferometer recombine at the beam splitter becomes
\begin{equation} \label{eq:totalElecField}
\begin{split}
\tilde{E}_o(\omega, t_p) &= \sqrt{T_s(\omega) R_m(\omega)} T_c(x, t_p) T_b(\omega) \tilde{E}_{in}(\omega, t_p) \\
&\quad \cdot \exp \Big\lbrace i[-\frac{\omega}{c} \cdot 2(L+\Delta L) - \Delta\varphi(x,t)] \Big\rbrace \\
...
&\quad \cdot \Big\lbrace \sqrt{T_s(\omega)} T_c(x, t_p) T_b(\omega) exp \lbrace i[-2\frac{\omega}{c}\Delta L  \\
&\quad - \Delta\varphi(x,t)] \rbrace - [1-T_b(\omega)] \Big\rbrace.
\end{split}
\end{equation}
...
\Delta\varphi(x,t_p) &\Rightarrow \Delta\varphi(\omega,t_p)
\end{align}
The intensity envelop then becomes
\begin{equation} \label{eq:totalIntensity}
\begin{split}
\tilde{I}_o(\omega, t_p) &\propto \Big\lbrace [1-T_b(\omega)]^2 + T_b(\omega)^2 T_c(\omega, t_p)^2 T_s(\omega)\Big\rbrace R_m(\omega) \\
& \cdot \lvert\tilde{E}_{in}(\omega, t_p)\rvert^2 - 2 [1-T_b(\omega)] T_c(\omega, t_p) T_b(\omega) R_m(\omega) \\
& \cdot \sqrt{T_s(\omega)} \lvert\tilde{E}_{in}(\omega, t_p)\rvert^2 cos[2\omega \Delta L / c + \Delta \varphi_c(\omega, t_p) ]
\end{split}
\end{equation}


\begin{figure*}
%\includegraphics[scale=0.2]{FigureOSA.jpg}
\caption{\label{fig:OSA} }
\end{figure*}

During time stretch, each frequency component $\omega$, or $\lambda$ will be one-to-one mapped into time domain. Define the relative time delay of $\lambda$ compared to central wavelength $\lambda_c$ as $t_i$, which is usually called intra-pulse time delay. Let us write in terms of $\lambda$, Eq.~\ref{eq:totalIntensity} can be simplified as
...
\end{equation}
where $I_b (\lambda, t_p)$ is the background / baseband intensity, and $I_i (\lambda, t_p)$ is the interference / intermediate envelop:
\begin{align} \label{eq:totalIntensityTwoTerms}
\begin{split}
I_b (\lambda, t_p) &= \Big\lbrace [1-T_b(\lambda)]^2 + T_b(\lambda)^2 T_c(\lambda, t_p)^2 T_s(\lambda)\Big\rbrace \\
&\cdot R_m(\lambda) \lvert \tilde{E}_{in}(\lambda, t_p)\rvert^2
\end{split}\\
\begin{split}
I_i (\lambda, t_p) &= - 2 [1-T_b(\lambda)] T_c(\lambda, t_p) T_b(\lambda) R_m(\lambda)\\
& \cdot \sqrt{T_s(\lambda)} \lvert \tilde{E}_{in}(\lambda, t_p)\rvert^2
\end{split}
\end{align}

Time stretch maps frequency domain into time domain by
\begin{equation} \label{eq:DCF}
t_i = D (\lambda - \lambda_c) L_f.
\end{equation}
where $\lambda_c$ is the central wavelength and $L_f$ the length of dispersive fiber. $D$ is the group velocity dispersion, that is, the temporal pulse spreading $\Delta t_i$ per unit bandwidth $\Delta\lambda$ per unit distance travelled. Thus the energy flux absorbed at the photodetector is the vertical cancatenation of inter-pulse waveforms:
\begin{equation}
\begin{split}
\tilde{I}_{PD}(t) &=\\
\text{vetcat} &
...
where \text{vetcat} means vertical concatenation. Each $\tilde{I}_{o}(t_i^{(n)}, t_p^{(m)})$ expresses the $n$th spatial pixel at the $m$th line image. Applying Eq.~\ref{eq:DCF} to Eq.~\ref{eq:totalIntensitylambda},
...
The second feature in Eq.~\ref{eq:afterDCF},
\begin{equation}
I_i (t_i^{(n)}, t_p^{(m)})cos\Big[\frac{4\pi D L_f\Delta L}{t_i^{(n)} + D\lambda_c L_f} + \Delta \varphi_c(t_i^{(n)}, t_p^{(m)})\Big]
\end{equation}
is a fast oscillating fringe, caused by the interference of the linearly chirped pulses multiplexed between the sample and reference arms in the Michelson interferometer. It can be separated by a band-pass filter in order to extract the object phase shift, $\Delta\varphi(x,t_p)$. The instantaneous phase of this feature can be readily retrieved from its analytic representation given by Hilbert transform, $\hilbert$:
\begin{equation}
\begin{split}
\angle \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) &=  \frac{4\pi D L_f\Delta L}{t_i^{(n)} + D\lambda_c L_f} + \Delta \varphi_c(t_i^{(n)}, t_p^{(m)}) \\
&= \arg\Big[\tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) + j \cdot \hilbert \lbrace \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) \rbrace\Big]
\end{split}
\end{equation}
...
\begin{equation}
\Delta\varphi(x,t_p^{(m)}) = \unwrap \lbrace\angle \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)})\rbrace - \frac{2\pi c}{\lambda_n} t_i^{(n)}
\end{equation}
...
\subsection{Calibration in Optical Loss Extraction}
...
\begin{equation}
OPD(x^{(n)},y^{(m)}) = \frac{\lambda(t_i^{(n)},t_p)}{2\pi} \Delta\varphi(t_i^{(n)},t_p^{(m)}) = \frac{\lambda(t_i^{(n)},t_p)}{2\pi} \Delta\varphi(t_i^{(n)},t_p^{(m)})
\end{equation}
