...
Lateral resoluiton of time stretch camera is decided by the limiting factors among diffraction limit of objective lens, resovability of the diffraction grating pairs, spectral resolution in amplified dispersive Fourier transform, photodetector rise-time and bandwidth, and the sampling rate of back-end digitizer. Details of the deterministic limiting factors for lateral resolution and evaluation of current TS-QPI system can be found in Table~\ref{tbl:Resolution}.
...
\multicolumn{1}{ |m{0.16\textwidth}  }{\multirow{2}{*}{Optical system} } &
...
Field of view (FOV) is the area covered by the interrogation rainbow when the rainbow pulses hit the imaging plane. The beam width of rainbow pulses is mainly decided by the optical bandwidth selected from the laser source, $\Delta\lambda$, the numerical apeture of objective lens, $NA$, as well as the dimensions and blaze angles of the diffraction gratings.

The resolution of phase measurement along axial direction is determined by the effective number of bits (ENOB) of the digitizer and affected by the noise of laser source. Since pulse to pulse intensity and phase fluactuation is small, noise from laser source is not the limiting factor for our phase measurement. Supposing the ENOB of the digitizer is $N$, the minimum detectable optical path length difference, $\Delta L$ can be estimated as
\begin{equation}
\frac{1}{2}sin(4\pi \Delta L/\lambda)=2^{-N}
\end{equation}
where $\lambda$ is the central wavelength of light. In our system, ENOB of the analog-to-digital converter is $5$. Thus OPD resolution along the axial direction is $\SI{7.9}{\nano\meter}$, corresponding to refractive index difference down to the level of $0.001$ for cellular level measurements.
