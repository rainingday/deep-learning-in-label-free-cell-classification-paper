...
From a set of seperate TS-QPI measurements, the cell transmittance, $T_s(\lambda)$, can be extracted, even when the transmittance of the solution, $T_s(\lambda)$, and the beam-splitter, $T_b(\lambda)$, are unknown. First, consider a measurement when there is no cell in the field of view (field of view is empty). In this case, Eq. \ref{eq:interferenceIntensity} becomes
\begin{align}
...
Next, one can block the sample arm and measure the signal coming back from only the reference arm:
\begin{equation} \label{eq:referenceIntensity}
I_r (\lambda, t_p) = [1-T_b(\lambda)]^2 R_m(\lambda) \lvert\tilde{E}_{in}(\lambda, t_p)\rvert^2
\end{equation}
Combining Eq. \ref{eq:backgroundIntensity}, \ref{eq:interferenceIntensityEmpty}, and \ref{eq:referenceIntensity}, and assuming that the input electric field pulse shape, $\lvert \tilde{E}_{in}(\lambda, t_p) \rvert$, is the same from pulse to pulse, the cell transmittance can be derived as
\begin{equation}
