...
There are two other major effects that may alter the phase extraction based on Hilbert Transform, that is, the inbalanced power distribution at the beam splitter in the Michaelson Interferometry, and the attenuation inbalance caused by microfluidic channel.

In addtional, since biological cells are semi-transparent, not only optical intensity will be altered by absorption and scattering when illumination light passes through the cells, but optical phase will also be influenced by the refractive index difference in the sample arm caused by the cells.
When taking the inbalanced power distribution at the beam splitter, the light splits into the two arms of the Michaelson Interferometry can be expressed as: 
\begin{equation}
Sample arm: E_s(t) = \sqrt{r}E_i(t)\exp(i\omega_{s} t)
\end{equation}
\begin{equation}
Reference arm: E_{LO}(t) = sqrt{1-r}E_i(t)\exp(i\omega_{LO} t)
\end{equation}
where $r$ is the reflectivity of the beam splitter. $E_s(t)$, $E_LO(t)$, $\omega_s$ and $\omega_{LO}$ are same as what mentioned above. 
