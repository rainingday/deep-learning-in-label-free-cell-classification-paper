...
There are two other major effects that may alter the quantitative phase extraction based on Hilbert Transform, that is, the inbalanced power distribution at the beam splitter in the Michaelson Interferometry, and the attenuation inbalance caused by microfluidic channel.
When taking the inbalanced power distribution at the beam splitter into consideration, the light splits into the two arms of the Michaelson Interferometry can be expressed as: 
\begin{equation}
\begin{split}
...
\begin{equation}
\Delta I_{total}^{(envelop)}(t) =
% \frac{E_i^2(t_p,\omega_{s}) [(1-r^2) + \alpha_s^2 \alpha_c^2(x,t) r^2] - E_i^2(t_p,\omega_{s}) [(1-r^2) + \alpha_s^2 r^2]}{E_i^2(t_p,\omega_{s}) [(1-r^2) + \alpha_s^2 r^2]}\\
 \frac{\alpha_s^2 r^2 [\alpha_c^2(x,t) - 1]}{1 - r^2 + \alpha_s^2 r^2}
\end{equation}
%Define $r^\prime = \frac{r}{}$, then the equation above can be written as
%\begin{equation}
%\Delta I_{total}^{(envelop)}(t) = \frac{(1-r^\prime)^2 (\alpha_c (x,t)^2 - 1)}{r^{\prime 2} + (1 - r^\prime)^2}
%\end{equation}
The ratio of amplitude change caused by interference to that of envelop change caused by cell absorption:
