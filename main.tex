...
&\quad \tilde{E}_s(\omega, t_p) = \sqrt{T_b} \tilde{E}_in(\omega, t_p)  \\
...
\end{split}
\end{equation}
where $L_0$ is the length of reference arm, and $\Delta L$ is the arm length mismatch between two arms. $t_d$ is the time delay during which rainbow flash travels from the beam splitter to the cell sample, $t_d = \frac{L_0 + \Delta L}{2}$. $T_s(\omega)$ is power transmittance of the surrounding buffer solutions and microfluidic channel, and $T_c(x,t_p + t_d)$ is spatial power transmittance of cells at location $x$ along the rainbow when being illuminated at time $t_p + t_d$. Both $T_s(\omega)$ and $T_c(x,t_p + t_d)$ have been multiplied twice since each rainbow flash pass through objects twice. Based on the spectral encoding setup, we know spatial information is encoded into specturm,
\begin{align}
T_c(x,t_p + t_d) &= T_c(\omega,t_p + t_d)\\
\Delta\varphi(x,t_p) &= \Delta\varphi(\omega,t_p)
\end{align}
...
& \cdot \lvert\tilde{E}_{in}(\omega, t_p)\rvert^2 - 2 [1-T_b(\omega)] T_c(\omega, t_p) T_b(\omega) \\
& \cdot R_m(\omega) \sqrt{T_s(\omega)} \lvert\tilde{E}_{in}(\omega, t_p)\rvert^2 cos[2\omega \Delta L / c + \Delta \varphi_c(\omega, t_p) ]
\end{split}
\end{equation}
...
\begin{equation}
I_i (t_i^{(n)}, t_p^{(m)})cos[ 4\pi D L_f\Delta L/(t_i^{(n)} + D\lambda_c L_f) + \Delta \varphi_c(t_i^{(n)}, t_p^{(m)})]
\end{equation}
...
\begin{equation}
\begin{split}
\angle \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) &=  4\pi D L_f\Delta L/(t_i^{(n)} + D\lambda_c L_f) + \Delta \varphi_c(t_i^{(n)}, t_p^{(m)}) \\
&= \arg\Big[\tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) \\
&+ j \cdot \hilbert \lbrace \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) \rbrace\Big]
\end{split}
\end{equation}
...
\begin{equation}
\Delta\varphi(x,t_p^{(m)}) = \unwrap(\angle \tilde{I}_{o}(t_i^{(n)}, t_p^{(m)}) - \lambda_n t_i^{(n)}
\end{equation}
