...
From a set of seperate TS-QPI measurements, the cell transmittance, $T_s$, can be extracted, even when the transmittance of the solution, $T_s$, and the beam-splitter, $T_b$, are unknown. First, consider a measurement when there is no cell in the field of view (field of view is empty). In this case, Eq. \ref{eq:backgroundIntensity} and \ref{eq:interferenceIntensity} become:
\begin{align}
\begin{split} \label{eq:backgroundIntensityEmpty}
I_b (\lambda, t_p^{(empty)}) &= \Big\lbrace [1-T_b(\lambda)]^2 + T_b^2(\lambda) T_s^2(\lambda) \Big\rbrace \\
&\cdot R_m(\lambda) \lvert \tilde{E}_{in}(\lambda, t_p^{(empty)})\rvert^2
\end{split}\\
...
Next, we can block the sample arm and measure the signal that is coming only from the reference arm:
\begin{equation} \label{eq:referenceIntensity}
I_r (\lambda, t_p) = [1-T_b(\lambda)]^2 R_m(\lambda) \tilde{E}_{in}(\lambda, t_p^{(empty)})\rvert^2
\end{equation}
Combining, Eq. \ref{eq:backgroundIntensity}, \ref{eq:backgroundIntensityEmpty}, \ref{eq:interferenceIntensityEmpty}, and \ref{eq:referenceIntensity}, the cell transmittance can be derived as
\begin{equation}
